...
# Uses inferno-data/blis/ for optimizer config and blis evaluator for all workloads.
...
kubectl apply -f "$EXP/configmap-blis-small.yaml"

echo "==> Deploying blis workloads (granite_8b/H100 Premium, llama_13b/H100 Bronze)"
kubectl apply -f "$EXP/dep-blis-granite.yaml"
kubectl apply -f "$EXP/dep-blis-llama.yaml"
