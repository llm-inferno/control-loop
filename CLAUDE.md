...
**Backend seam** (`pkg/backend`): the Collector's sense and the Actuator's actuate operations are two interfaces (`Sensor`, `Actuator`) selected per-process by `INFERNO_BACKEND` — `serversim` (default) or `llmd`. The `serversim` backend is what the rest of this section describes (server-sim `/latest` + label actuation). The `llmd` backend targets a real llm-d deployment: it senses **entirely from Prometheus** keyed on discovered pod names (no server-sim sidecar, no coherence gate, `ArrivalRate := Throughput`), and its actuator patches `/spec/replicas` only (m\* pinned via `DEFAULT_MAX_BATCH_SIZE`). See [`docs/superpowers/specs/2026-07-01-llmd-backend-integration-design.md`](docs/superpowers/specs/2026-07-01-llmd-backend-integration-design.md) (incl. the *As-built reconciliation* section) and the `INFERNO_PROMETHEUS_*` vars in [`docs/env-vars.md`](docs/env-vars.md).
...
The full environment-variable reference (defaults and descriptions) is in [`docs/env-vars.md`](docs/env-vars.md). Notable knobs: `INFERNO_CONTROL_PERIOD`, `INFERNO_CONTROL_DYNAMIC`, `TUNER_HOST` (enables the Tuner), `SERVERSIM_CONTINUOUS` / `SERVERSIM_SATURATION_POLICY` (continuous traffic generator; `SERVERSIM_CONTINUOUS=false` is non-continuous on-demand `/latest` for simulator backends), `DEFAULT_MAX_BATCH_SIZE` (concurrency-search switch — see [`docs/concurrency-control.md`](docs/concurrency-control.md)), `INFERNO_CALIBRATION_ENABLED` (benchmarking-on-the-fly calibration — see [`docs/calibration.md`](docs/calibration.md)), `WATCH_NAMESPACE` (shared-cluster scoping), and `INFERNO_BACKEND` (`serversim` default | `llmd` — selects the real-llm-d all-Prometheus backend; see `docs/superpowers/specs/2026-07-01-llmd-backend-integration-design.md`).
