...
**Saturated pod re-simulation**: When a pod's simulation result has a non-empty `Saturation` field (`bandwidth`, `kv_capacity`, or `overloaded`), the resulting TTFT/ITL are degenerate and not useful for EKF tuning. The Collector re-simulates at progressively lower load — `0.90 → 0.75 → 0.60 × MaxRPS` — stopping as soon as the result is unsaturated (`overloadMaxRetries = 3`, `overloadTargetUtilization = 0.90`, `overloadRetryStep = 0.15` in `pkg/collector/collector.go`). Both `ArrivalRate` and `Throughput` in the replicaSpec are set to the adjusted rate. If saturation persists after all attempts or a re-simulation errors, the pod is excluded from `ReplicaSpecs` entirely (same effect as a simulation failure).
