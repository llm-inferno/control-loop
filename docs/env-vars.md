...
| `INFERNO_CALIBRATION_ENABLED` | `false` | Enable benchmarking-on-the-fly calibration. When `true`, after each `/tune` the controller asks the tuner (`GET /calibration-status`) whether any `(model, accelerator)` pair needs calibration — i.e. natural warm-up excitation was insufficient (ill-conditioned fit) and the pair has not been calibrated yet. If so it drives a short load sweep (collector `GET /sweep`) and POSTs the points to the tuner's `/calibrate`. Requires the tuner enabled (`TUNER_HOST` set). See [`docs/calibration.md`](calibration.md). |
| `INFERNO_CALIB_RPM_FACTORS` | `0.25,0.5,0.75,1.0` | Comma-separated multipliers of the deployment's nominal RPM that form the sweep's arrival-rate ramp (read by the Collector). Skewed **below** nominal so points stay unsaturated on high-nominal workloads (where load is already near the single-replica knee and higher multiples would saturate and be dropped). Two extra points at the **lowest** ramp rate with a wide 4× input/output token-ratio swing are always appended to separate the prefill (`beta`) and decode (`gamma`) terms. |
| `INFERNO_CALIB_POINT_TIMEOUT_SEC` | `120` | Per-sweep-point timeout for the Collector's `POST /simulate` + poll against the pod's server-sim sidecar. The blis DES solve can be slow at high load; the default is generous. |
| `INFERNO_CALIB_POLL_INTERVAL_SEC` | `2` | Interval at which the Collector polls `GET /simulate/:id` for each sweep point's result. |
